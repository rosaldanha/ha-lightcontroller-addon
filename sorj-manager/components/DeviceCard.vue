...
const handleBackup = async () => {
  ...
  try {
    // 1. Chama a API
    const data: any = await (<Blob>$fetch("api/backup", {
      method: "POST",
...
<template>
  ...
  >
    <div class="p-4 flex justify-between items-start mb-auto">
      ...
      >
        <span v-if="device.isOnline" class="text-esphome-success">Online</span>
        <span v-else class="text-esphome-error">Offline</span>
      </div>
    </div>

    <div class="bg-[#222222] px-2 py-2 flex items-center justify-end space-x-2">
      <button
