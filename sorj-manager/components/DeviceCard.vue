...
const handleBackup = async () => {
    ...
    try {
        ...
        const response = await fetch("./api/backup", {
            ...
            body: JSON.stringify({
                deviceName: props.device.substitutions.device_name,
            }),
        });
        ...
        link.setAttribute(
            "download",
            `backup-${props.device.substitutions.device_name}-${dateStr}.zip`,
        );
...
<template>
    <div
        ...
        :class="[
            false ? 'border-esphome-success' : 'border-esphome-error', // device.isOnline hardcoded to false
        ]"
    >
        <div class="p-4 flex justify-between items-start mb-auto">
            <div class="overflow-hidden">
                <h3
                    ...
                    :title="device.substitutions.device_name"
                >
                    {{ device.substitutions.device_name }}
                </h3>
                <p class="text-sm text-gray-400 truncate" title="any">any</p>
            </div>

            <div
                class="flex items-center space-x-1 text-xs font-bold uppercase tracking-wider flex-shrink-0 ml-2"
            >
                <!-- <span v-if="false" class="text-esphome-success">Online</span> -->
                <span class="text-esphome-error">Offline</span>
            </div>
        </div>
